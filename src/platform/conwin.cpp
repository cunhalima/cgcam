// Copiado do código-fonte do Quake 3 (GPL)
...
LRESULT WINAPI InputLineWndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		if ( wParam == 13 )
		{
			...
			//Sys_Print( va( "]%s\n", inputBuffer ) );
			//Syscon_Print(inputBuffer);
			//Syscon_Print("\n");
			con_type(inputBuffer);
			return 0;
		} else if ( wParam == 27 ) {
...
** Conbuf_AppendText
*/
void Syscon_Print(const char *pMsg) {
...
static int m_argc;
static char *m_argv[32];

void buildargs() {
	int argc;
	LPWSTR wstr = GetCommandLineW();
	LPWSTR* argv = CommandLineToArgvW(wstr, &argc);
	...
	for (i = 0; i < argc; i++) {
		...
		m_argv[m_argc] = &cmdline[j];
		m_argc++;
		for (wchar_t *wc = argv[i]; *wc != 0; wc++) {
	...
	//char ss[1000];
	//sprintf_s(ss, sizeof(ss), "%d %d %d", i, j, argc);
	//if (m_argc != argc) {
	//	SYS_ShowError(ss);
	//}
}
