...
void SV_HitEntity(svEntity_t *e) {
	assert(IS_VALID_ENTITY_ID(e->id));
...
void SV_Think(svEntity_t *e) {
	...
	if (e->id < MAX_CLIENTS) {
		svPlayer_t *p = SV_GetPlayerFromEntity(e);
		if (p->lastFrame == 0) {
			p->lastFrame = FRAME_stand40;
			e->frame = FRAME_stand01;
			return;
		}
		// check if we have to switch anim
		if (p->pain) {
			if (!(p->lastFrame >= FRAME_pain101 && p->lastFrame <= FRAME_pain304)) {
				int baseFrame = FRAME_pain101 + (rand() % 3) * (FRAME_pain201 - FRAME_pain101);
				p->lastFrame = baseFrame + 3;
				e->frame = baseFrame - 1;
			}
		} else if (p->attack) {
			if (p->lastFrame != FRAME_attack8 && p->attack) {
				p->lastFrame = FRAME_attack8;
				e->frame = FRAME_attack1 - 1;
			}
		} else {
			if (p->lastFrame != FRAME_stand40 && !p->moving) {
				p->lastFrame = FRAME_stand40;
				e->frame = FRAME_stand01 - 1;
			} else if (p->lastFrame != FRAME_run6 && p->moving) {
				p->lastFrame = FRAME_run6;
				e->frame = FRAME_run1 - 1;
			}
		}
#if 0
		if (p->lastFrame != FRAME_pain104 && p->pain) {
			p->lastFrame = FRAME_pain104;
			e->frame = FRAME_pain101 - 1;
		} else if (p->lastFrame != FRAME_attack8 && p->attack) {
			p->lastFrame = FRAME_attack8;
			e->frame = FRAME_attack1 - 1;
		} else if (p->lastFrame != FRAME_stand40 && !p->moving) {
			p->lastFrame = FRAME_stand40;
			e->frame = FRAME_stand01 - 1;
		} else if (p->lastFrame != FRAME_run6 && p->moving) {
			p->lastFrame = FRAME_run6;
		...
		if (e->frame < p->lastFrame) {
			e->frame++;
		} else {
			if (p->lastFrame == FRAME_stand40) {
				e->frame = FRAME_stand01;
			} else if (p->lastFrame == FRAME_run6) {
				e->frame = FRAME_run1;
			} else if (p->lastFrame >= FRAME_pain101 && p->lastFrame <= FRAME_pain304) {
				e->frame = FRAME_stand01;
				p->pain = false;
			} else {
				e->frame = FRAME_stand01;
